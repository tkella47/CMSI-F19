...
import static spellex.SpellEx.editDistance;
import java.io.File;
...
public class SpellExTests {
    ...
    public static void populateDictFromFile (String filename, Map<String, Integer> dict) throws FileNotFoundException {
        ...
        File file = new File("./src/spellex/" + filename);
    ...
    public void LeastDistantTest_t1() {
        SpellEx se = new SpellEx(tinyDict);
        assertEquals(new HashSet<String>(Arrays.asList("bat","ab")), se.getNLeastDistant("ba", 2));
        assertEquals(new HashSet<String>(Arrays.asList("cat", "bat")), se.getNLeastDistant("dat", 2));
    }
    ...
    public void NBestUnderDistanceTest_t1() {
        SpellEx se = new SpellEx(tinyDict);
        assertEquals(new HashSet<String>(Arrays.asList("dog", "cat")), se.getNBestUnderDistance("dat", 2, 2));
    ...
    public void NBestUnderDistanceTest_big_t2() {
        SpellEx se = new SpellEx(bigDict);
        assertEquals(new HashSet<String>(Arrays.asList("for", "found", "words","form","front")), se.getNBestUnderDistance("forns", 5, 2));
        assertEquals(new HashSet<String>(Arrays.asList("educated","educate","emulate","equate","dedicate")), se.getNBestUnderDistance("educate", 5, 2));
    }
    
    @Test
    public void NBestUnderDistanceTest_big_t3() {
        SpellEx se = new SpellEx(bigDict);
        assertEquals(new HashSet<String>(Arrays.asList("looking", "cooking", "looming","locking","booking")), se.getNBestUnderDistance("loooking", 5, 2));
        assertEquals(new HashSet<String>(Arrays.asList("high","height","heath","hiett","heigh")), se.getNBestUnderDistance("hiegth", 5, 2));
    }
    
    /*       
     *  Set<String> setr = se.getNBestUnderDistance("fihsnig",5, 3);
        String[] resultArr = new String[setr.size()];
        resultArr = setr.toArray(resultArr);
        for(String str : resultArr) {
            System.out.println(str + " freq " + bigDict.get(str));
        } 
     */
